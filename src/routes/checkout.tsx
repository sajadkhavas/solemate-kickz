...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [{ title: "Checkout نمایشی — SOLE" }, { name: "robots", content: "noindex, follow" }],
  }),
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      >
        <div className="max-w-3xl">
          ...
          <h1
            id="checkout-heading"
            className="mt-2 font-display text-4xl font-black tracking-tight sm:text-5xl md:text-6xl"
          >
            Checkout نمایشی
          </h1>
          <p className="mt-4 font-fa text-sm leading-7 text-muted-foreground">
            این صفحه اطلاعات را فقط در مرورگر اعتبارسنجی و برای Review آماده می‌کند. هیچ سفارش واقعی
            ایجاد نمی‌شود و هیچ سرویس ارسال یا پرداختی متصل نیست.
          </p>
        </div>

        {!hasHydrated || !draftHydrated ? (
          <div
            data-testid="checkout-hydrating"
          ...
          <ErrorState
            ...
            action={
              <Link to="/cart" className="btn-hype">
                بازگشت به سبد
              </Link>
            }
          />
          ...
          <div className="mt-10 grid min-w-0 gap-8 xl:grid-cols-[minmax(0,1.2fr)_minmax(22rem,0.8fr)]">
            ...
            >
              {Object.keys(errors).length ? (
                ...
                >
                  <h2 id="checkout-error-summary-title" className="font-fa font-bold">
                    بعضی اطلاعات نیاز به اصلاح دارند
                  </h2>
                  <ul className="mt-2 space-y-1 font-fa text-sm">
                    {(Object.entries(errors) as Array<[CheckoutField, string]>).map(
                      ([field, message]) => (
                        <li key={field}>
                          <button
                            type="button"
                            className="min-h-11 text-start underline underline-offset-4"
                            onClick={() => document.getElementById(`checkout-${field}`)?.focus()}
                          >
              ...
              >
                ...
                <div className="mt-6 grid gap-5 sm:grid-cols-2">
                  ...
                  <Field
                    ...
                    error={errors.phone}
                    help="ارقام فارسی و لاتین پذیرفته می‌شوند."
                  >
                    <Input
                      id="checkout-phone"
                      name="phone"
                      ...
                      aria-describedby={
                        errors.phone ? "checkout-phone-error" : "checkout-phone-help"
                      }
                      className="h-11 min-h-11 text-left"
              ...
              <section
                ...
                className="rounded-2xl border border-border bg-surface p-4 sm:p-6"
              >
                <SectionHeading icon={<MapPin />} eyebrow="Address" id="checkout-address-heading">
                  نشانی
                </SectionHeading>
                <p className="mt-2 font-fa text-xs leading-6 text-muted-foreground">
                  Dataset یا API معتبر استان/شهر در پروژه وجود ندارد؛ بنابراین فیلدها آزاد هستند و
                  هیچ فهرست ساختگی نمایش داده نمی‌شود.
                </p>
                <div className="mt-6 grid gap-5 sm:grid-cols-2">
                  <Field
                    label="استان/منطقه"
                    required
                    inputId="checkout-province"
                  ...
                  <div className="sm:col-span-2">
                    <Field
                      ...
                      help="آدرس‌های طولانی فارسی و متن ترکیبی RTL/LTR پشتیبانی می‌شوند."
                    >
                      <Textarea
                        id="checkout-address"
                        name="address"
                        ...
                        aria-describedby={
                          errors.address ? "checkout-address-error" : "checkout-address-help"
                        }
                  ...
                  <Field label="پلاک" optional inputId="checkout-plate">
                    <Input
                      id="checkout-plate"
                      name="plate"
                      dir="auto"
                  ...
                  <Field label="واحد" optional inputId="checkout-unit">
                    <Input
                      id="checkout-unit"
                      name="unit"
                      dir="auto"
                  ...
                  >
                    <Input
                      ...
                      onBlur={(event) =>
                        updateField("postalCode", normalizeDigits(event.target.value))
                      }
              ...
              <Boundary
                ...
                testId="checkout-delivery-boundary"
                eyebrow="Delivery boundary"
                title="ارسال"
              >
                هیچ گزینه، هزینه یا زمان تحویل در این نسخه ساخته نمی‌شود. انتخاب ارسال زمانی فعال
                می‌شود که Backend یا سیاست معتبر فروشگاه داده واقعی ارائه کند.
              </Boundary>
              <Boundary
                id="checkout-payment-heading"
                testId="checkout-payment-boundary"
                eyebrow="Payment boundary"
                title="پرداخت"
              >
                هیچ کارت بانکی، درگاه، لوگوی پرداخت یا تراکنش نمایشی ساخته نشده است. اتصال پرداخت
                واقعی به Backend آینده وابسته است.
              </Boundary>

              <button
                type="submit"
                className="btn-hype min-h-14 w-full justify-center font-fa"
                data-testid="checkout-review-submit"
              >
            ...
            <aside className="min-w-0 space-y-6 xl:sticky xl:top-28 xl:self-start">
              ...
              >
                ...
                <div className="mt-5 space-y-4">
                  {readyItems.map((item) => {
                    ...
                    return (
                      <article
                        key={item.key}
                        data-testid="checkout-review-item"
                        className="grid min-w-0 grid-cols-[4.5rem_minmax(0,1fr)] gap-3 border-b border-border pb-4 last:border-b-0 last:pb-0"
                      >
                        <CartProductImage
                          src={item.shoe.image}
                          alt={label}
                          testId="checkout-item-image"
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 font-fa text-xs text-muted-foreground">
                            سایز <bdi dir="ltr">{item.size}</bdi> · تعداد{" "}
                            <bdi dir="ltr">{item.qty}</bdi>
                          </p>
                          <p className="mt-2 font-mono-num text-sm font-semibold">
                ...
                <div className="mt-5 flex items-center justify-between gap-4 border-t border-border pt-4">
                  ...
                  <span className="font-mono-num text-lg font-bold" data-testid="checkout-subtotal">
                    {formatPrice(subtotal)}
                  </span>
                </div>
                <p className="mt-3 font-fa text-[11px] leading-5 text-muted-foreground">
                  این عدد فقط جمع قیمت فعلی اقلام Dataset است؛ هزینه ارسال، مالیات احتمالی و مبلغ
                  نهایی سفارش را شامل نمی‌شود.
                </p>
              </section>
              ...
              {reviewReady ? (
                <section
                  ...
                  className="rounded-2xl border border-primary/50 bg-primary/5 p-5 outline-none focus-visible:ring-2 focus-visible:ring-focus sm:p-6"
                >
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                      <p className="eyebrow text-primary">Validated locally</p>
                      <h2
                  ...
                  <dl className="mt-5 space-y-4 font-fa text-sm">
                    <div>
                      <dt className="font-bold">مشتری</dt>
                      <dd className="mt-1 break-words text-muted-foreground" dir="auto">
                        {draft.firstName.trim()} {draft.lastName.trim()}
                      </dd>
                      <dd className="mt-1 text-muted-foreground">
                        <bdi dir="ltr">{normalizePhone(draft.phone)}</bdi>
                        {draft.email.trim() ? (
                          <>
                            {" "}
                            · <bdi dir="ltr">{draft.email.trim()}</bdi>
                          </>
                        ) : null}
                      </dd>
                    </div>
                    <div className="border-t border-border pt-4">
                      ...
                      <dd
                        ...
                        dir="auto"
                      >
                        {draft.province.trim()}، {draft.city.trim()}، {draft.address.trim()}
                        {draft.plate.trim() ? `، پلاک ${draft.plate.trim()}` : ""}
                        {draft.unit.trim() ? `، واحد ${draft.unit.trim()}` : ""}
                      </dd>
                      {draft.postalCode.trim() ? (
                        <dd className="mt-1 text-muted-foreground">
                          کدپستی: <bdi dir="ltr">{normalizeDigits(draft.postalCode.trim())}</bdi>
                        </dd>
                      ) : null}
                    </div>
                    <div className="border-t border-border pt-4">
                      ...
                      <dd className="mt-1 text-muted-foreground">
                        هنوز روش، هزینه یا زمان معتبر از Backend دریافت نشده است.
                      </dd>
                    </div>
                    <div className="border-t border-border pt-4">
                      <dt className="font-bold">پرداخت</dt>
                      <dd className="mt-1 text-muted-foreground">
                        سرویس پرداخت واقعی متصل نیست و هیچ تراکنشی آغاز نمی‌شود.
                      </dd>
                    </div>
                  </dl>
                  <button
                    type="button"
                    disabled
                    aria-describedby="checkout-final-action-help"
                    className="mt-6 min-h-14 w-full rounded-full border border-border bg-interactive px-5 font-fa font-bold text-muted-foreground"
                    data-testid="checkout-final-action"
                  >
                    ادامه پس از اتصال سرویس سفارش
                  </button>
                  <p
                    id="checkout-final-action-help"
                    className="mt-2 font-fa text-xs leading-6 text-muted-foreground"
                  >
                    این کنترل عمداً غیرفعال است؛ در این فاز سفارش واقعی ثبت نمی‌شود.
                  </p>
                </section>
              ) : (
                <section
                  aria-labelledby="checkout-review-placeholder-heading"
                  className="rounded-2xl border border-dashed border-border bg-surface p-5 sm:p-6"
                  data-testid="checkout-review-placeholder"
                >
                  <h2
                    id="checkout-review-placeholder-heading"
                    className="font-display text-xl font-bold"
                  >
                    Review هنوز آماده نیست
                  </h2>
                  <p className="mt-2 font-fa text-sm leading-7 text-muted-foreground">
                    فرم را کامل و «بررسی اطلاعات واردشده» را انتخاب کنید تا خلاصه مشتری، آدرس و
                    مرزهای ارسال/پرداخت نمایش داده شود.
                  </p>
                </section>
              )}
            </aside>
...
function SectionHeading({
  ...
  return (
    <div className="flex items-start gap-3">
      <span aria-hidden="true" className="mt-1 size-5 shrink-0 text-primary">
        {icon}
      </span>
      <div>
        <p className="eyebrow text-primary">{eyebrow}</p>
        <h2 id={id} className="mt-1 font-display text-2xl font-bold">
          {children}
        </h2>
      </div>
    </div>
...
function Boundary({
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 font-fa text-sm leading-7 text-muted-foreground">{children}</p>
    </section>
  );
}
...
function Field({
  ...
  return (
    <div className="min-w-0">
      <label
        htmlFor={inputId}
        className="flex min-h-6 items-center gap-2 font-fa text-sm font-bold"
      >
        {label}
        {required ? <span className="text-danger">الزامی</span> : null}
        {optional ? <span className="font-normal text-muted-foreground">اختیاری</span> : null}
      </label>
      <div className="mt-2">{children}</div>
      {error ? (
        <p id={`${inputId}-error`} className="mt-1 font-fa text-xs leading-5 text-danger">
          <span aria-hidden="true">● </span>
          {error}
        </p>
      ) : help ? (
        <p id={`${inputId}-help`} className="mt-1 font-fa text-xs leading-5 text-muted-foreground">
          {help}
        </p>
      ) : null}
    </div>
