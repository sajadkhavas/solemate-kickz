  ...
  if (session === undefined) {
    return (
      <AccountShell>
        <div
          role="status"
          className="grid min-h-[24rem] place-items-center font-fa text-muted-foreground"
        >
          در حال دریافت حساب…
        </div>
      </AccountShell>
        ...
        <div className="grid min-h-[26rem] place-items-center rounded-[var(--radius-xl)] border border-dashed border-border-strong bg-surface p-8 text-center">
          <div className="max-w-xl">
            ...
            <p className="mt-3 font-fa leading-7 text-muted-foreground">
              اطلاعات حساب فقط پس از ورود امن Google از سرور دریافت می‌شود.
            </p>
      ...
      <AccountShell>
        <div className="grid min-h-[26rem] place-items-center rounded-[var(--radius-xl)] border border-warning/40 bg-warning/5 p-8 text-center">
          <div className="max-w-xl">
            ...
            <p className="mt-3 font-fa leading-7 text-muted-foreground">
              برای استفاده از بخش حساب، شماره همراه خود را تکمیل کنید.
            </p>
            <Button asChild size="lg" className="mt-6">
              <a href="/auth?complete=phone">تکمیل شماره همراه</a>
            </Button>
          </div>
        </div>
      </AccountShell>
  ...
  return (
    <AccountShell>
      ...
      {status ? (
        <p
          role="status"
          className="mt-4 rounded-[var(--radius-md)] border border-warning/40 bg-warning/5 p-3 text-sm"
        >
          {status}
        </p>
      ) : null}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[16rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          {section === "profile" ? (
            <Profile session={session} onRefresh={refresh} onStatus={setStatus} />
          ) : null}
          {section === "addresses" ? (
            <Addresses addresses={addresses} onRefresh={refresh} onStatus={setStatus} />
          ) : null}
          {section === "orders" ? <Orders orderId={orderId} /> : null}
        </div>
      </div>
    </AccountShell>
  );
...
function AccountShell({ children }: { children: React.ReactNode }) {
  return (
    <div
      ...
      data-testid="production-account-page"
    >
      <Navbar />
...
function Overview({
  ...
  const deletion = async () => {
    try {
      ...
      onStatus(
        deletionPending
          ? "درخواست حذف حساب لغو شد."
          : "درخواست حذف حساب ثبت شد و تا زمان اجرای کنترل‌شده قابل لغو است.",
      );
    } catch {
    ...
    <div className="space-y-6" data-testid="production-account-overview">
      ...
      <section className="rounded-[var(--radius-xl)] border border-border bg-surface p-6">
        ...
        <p className="mt-2 font-fa leading-7 text-muted-foreground">
          تاریخچه رضایت‌ها append-only است و خروجی داده حساب مستقیماً از Backend ساخته می‌شود.
        </p>
        <div className="mt-5 flex flex-wrap gap-3">
          <Button asChild variant="outline">
            <a href={accountExportUrl()}>
              <Download aria-hidden="true" />
              دریافت خروجی حساب
            </a>
          </Button>
          {!privacy?.granted ? (
            <Button type="button" variant="outline" onClick={setPrivacy}>
              <ShieldCheck aria-hidden="true" />
              ثبت رضایت حریم خصوصی
            </Button>
          ) : null}
        </div>
      </section>
      ...
      <section className="rounded-[var(--radius-xl)] border border-danger/30 bg-danger/5 p-6">
        ...
        <p className="mt-2 font-fa leading-7 text-muted-foreground">
          {deletionPending
            ? "درخواست حذف ثبت شده است. تا قبل از اجرای نهایی می‌توانید آن را لغو کنید."
            : "درخواست حذف، حساب را بلافاصله پاک نمی‌کند؛ اجرای نهایی به‌صورت کنترل‌شده و قابل حسابرسی انجام می‌شود."}
        </p>
        <Button
          type="button"
        ...
        >
          ...
          {deletionPending ? "لغو درخواست حذف" : "درخواست حذف حساب"}
        </Button>
      </section>
    </div>
  );
...
function Profile({
  ...
  return (
    <form
      ...
      data-testid="production-profile-form"
    >
      ...
      <div className="mt-6 grid gap-5">
        ...
        <Field
          label="شماره همراه"
          name="phone"
          defaultValue={session.phone_e164 ?? ""}
          type="tel"
          autoComplete="tel"
          dir="ltr"
        />
      ...
      <p className="mt-3 text-sm text-muted-foreground">
        تغییر شماره، وضعیت تأیید قبلی آن را پاک می‌کند. OTP تا فعال‌سازی رسمی سرویس غیرفعال است.
      </p>
      <Button type="submit" className="mt-6" loading={saving} loadingLabel="در حال ذخیره">
        ذخیره پروفایل
      </Button>
    </form>
  );
    ...
    <div className="space-y-6" data-testid="production-addresses">
        ...
        {addresses.map((address) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-semibold">{address.label || "آدرس"}</p>
                <p className="mt-1 text-sm text-muted-foreground">{address.recipient_name}</p>
              </div>
              {address.is_default ? (
                <span className="rounded-full bg-interactive px-2 py-1 text-xs text-primary">
                  پیش‌فرض
                </span>
              ) : null}
            </div>
            <p className="mt-4 font-fa text-sm leading-6 text-muted-foreground">
            ...
            <div className="mt-4 flex gap-2">
              ...
              <Button type="button" size="sm" variant="ghost" onClick={() => remove(address.id)}>
                حذف
              </Button>
            </div>
          </article>
      ...
      >
        ...
        <div className="mt-5 flex gap-3">
          <Button type="submit" loading={saving} loadingLabel="در حال ذخیره">
            ذخیره آدرس
          </Button>
          {editing ? (
...
function Orders({ orderId }: { orderId?: string }) {
  ...
  if (orders === undefined)
    return (
      <div role="status" className="rounded-[var(--radius-xl)] border border-border p-8">
    ...
    return (
      <section className="rounded-[var(--radius-xl)] border border-border bg-surface p-6">
        ...
        <h2 className="mt-4 font-display text-2xl font-black">
          سفارش <bdi dir="ltr">{selected.id}</bdi>
        </h2>
        <p className="mt-2 text-muted-foreground">وضعیت: {selected.status}</p>
        ...
        <p className="mt-6 text-lg font-bold">
          مبلغ نهایی: {formatPrice(selected.total_minor / 10)}
        </p>
      </section>
    );
  if (!orders.length)
    return (
      <div className="rounded-[var(--radius-xl)] border border-dashed border-border p-8 text-center">
  ...
  return (
    <section>
      <h2 className="font-display text-3xl font-black">سفارش‌ها</h2>
      <div className="mt-6 space-y-4">
        {orders.map((order) => (
          <Link
            ...
            className="block min-h-11 rounded-[var(--radius-xl)] border border-border bg-surface p-5"
          >
            <div className="flex flex-wrap justify-between gap-3">
              ...
              <strong>{formatPrice(order.total_minor / 10)}</strong>
            </div>
          </Link>
        ))}
      </div>
...
function Summary({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-[var(--radius-xl)] border border-border bg-surface p-5">
      ...
      <p className="mt-2 font-semibold" dir={title.includes("همراه") ? "ltr" : undefined}>
        {value}
      </p>
    </div>
  );
  ...
  return (
    <label className="block text-sm font-medium">
      ...
      <input
        {...props}
        name={name}
        className="mt-2 min-h-12 w-full rounded-[var(--radius-md)] border border-input bg-background px-4 outline-none focus:border-primary disabled:opacity-60"
      />
    </label>
  );
}
