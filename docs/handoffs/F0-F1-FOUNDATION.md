...
The complete `artifacts/` path is ignored. Runtime reports and screenshots are generated only inside CI. Every run publishes the exact commit and generated-report inventory to the immutable GitHub Actions step summary. ZIP Artifact publication is optional behind the Repository variable `ENABLE_QA_ARTIFACT_UPLOAD=true` because the account-level Actions Artifact quota is currently exhausted.
...
- Exact commit and generated-report inventory always written to the immutable GitHub Actions step summary
- Optional one-day ZIP Artifact publication only when `ENABLE_QA_ARTIFACT_UPLOAD=true`
