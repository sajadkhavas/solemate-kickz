...
for (const [name, command] of Object.entries(expectedScripts)) {
  if (pkg.scripts?.[name] !== command)
    failures.push(`package script ${name} is missing or changed`);
}
...
requireMatch(
  "VPS build must opt in through SOLE_DEPLOY_TARGET",
  vite,
  /SOLE_DEPLOY_TARGET\s*===\s*["']node-server["']/,
);
requireMatch(
  "Nitro node-server preset must be conditional",
  vite,
  /isNodeServerBuild[\s\S]*preset:\s*["']node-server["']/,
);
...
requireMatch(
  "preflight must require ss before checking the port",
  preflight,
  /for command in[^\n]*\bss\b/,
);
...
requireMatch(
  "local Node bootstrap must verify checksum",
  bootstrapNode,
  /SHASUMS256\.txt[\s\S]*sha256sum/,
);
requireMatch(
  "Node wrapper must execute local runtime",
  nodeWrapper,
  /\.runtime\/node\/bin\/node|RUNTIME.*node/s,
);
...
requireMatch(
  "safe dependency install must isolate HOME",
  installSafe,
  /--setenv="HOME=\$HOME_DIR"/,
);
requireMatch(
  "safe dependency install must isolate Bun cache",
  installSafe,
...
requireMatch(
  "VPS build must set node-server target",
  buildNode,
  /SOLE_DEPLOY_TARGET:\s*"node-server"/,
);
requireMatch(
  "VPS build must verify Nitro server output",
  buildNode,
...
requireMatch(
  "deployment docs must forbid dev server as production",
  docs,
  /Do not run `vite dev` or `vite preview` as the production service/i,
);
...
requireMatch(
  "frontend workflow must run deployment audit",
  workflow,
  /VPS deployment contract audit[\s\S]*audit:deploy/,
);
requireMatch(
  "frontend workflow must build Node-server output",
  workflow,
  /VPS Node-server build[\s\S]*build:vps/,
);
