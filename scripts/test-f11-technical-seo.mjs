...
function checkHead(name, page, { robots, canonical, requireSocial = true }) {
  ...
  record(`${name}: canonical policy`, canonical ? canonicals.length === 1 && canonicals[0] === canonical : canonicals.length === 0, canonicals);
  if (requireSocial) {
  ...
  if (canonical) record(`${name}: matching og:url`, ogUrls.length === 1 && ogUrls[0] === canonical, ogUrls);
  record(`${name}: no localhost metadata`, !/localhost|127\.0\.0\.1|0\.0\.0\.0/i.test(page.head), null);
}
...
async function run(baseUrl) {
  ...
  for (const [name, path, canonical] of publicPages) {
    ...
    record(`${name}: Persian RTL document`, /<html[^>]*lang=["']fa["'][^>]*dir=["']rtl["']/i.test(page.body), null);
  }
  ...
  for (const path of [
    ...
    const page = await fetchPage(baseUrl, path);
    record(`${path}: HTTP 200`, page.response.status === 200, page.response.status);
  ...
  record("catalog exposes crawlable SSR product anchors", productLinks.length > 0, productLinks.slice(0, 5));
  record("catalog has meaningful SSR heading", /<h1\b[^>]*>[\s\S]*?<\/h1>/i.test(catalog.body), null);
  ...
  for (const path of ["/", "/about", "/brands", "/products", "/product/1"]) {
    ...
    for (const raw of jsonLdValues(page.head)) {
      try {
        ...
        record(`${path}: JSON-LD parses`, false, error instanceof Error ? error.message : String(error));
      }
    }
  }
  ...
  record("home exposes WebSite JSON-LD", allJsonLd.some((value) => value?.["@type"] === "WebSite"), allJsonLd);
  record(
  ...
  for (const path of utilityPaths) {
    ...
    record(`${path}: not canonicalized to homepage`, !canonicalValues(page.head).includes(`${SITE_URL}/`), canonicalValues(page.head));
  }

  for (const path of ["/product/invalid-id", "/product/", "/this-route-does-not-exist"]) {
    const page = await fetchPage(baseUrl, path);
    record(`${path}: real 404 status`, page.response.status === 404, page.response.status);
    record(
  ...
  record("robots.txt valid allow policy", /^User-agent: \*$/m.test(robotsBody) && /^Allow: \/$/m.test(robotsBody), robotsBody);
  record("robots.txt does not blanket block", !/^Disallow: \/$/m.test(robotsBody), robotsBody);
  record("robots.txt advertises configured sitemap", robotsBody.includes(`Sitemap: ${SITE_URL}/sitemap.xml`), robotsBody);

  const sitemap = await fetch(`${baseUrl}/sitemap.xml`);
  const sitemapBody = await sitemap.text();
  const locs = [...sitemapBody.matchAll(/<loc>([^<]+)<\/loc>/g)].map((match) => match[1]);
  record("sitemap HTTP 200 XML", sitemap.status === 200 && /application\/xml/.test(sitemap.headers.get("content-type") ?? ""), sitemap.status);
  record(
  ...
  record("sitemap excludes utility, catalog queries and demo products", !locs.some((url) => /cart|checkout|wishlist|account|auth|products|product\//.test(url)), locs);
...
withF11Server(
  ...
  record("F11 runtime suite completed", false, error instanceof Error ? error.stack ?? error.message : String(error));
  writeReport("artifacts/reports/f11-technical-seo-runtime.json", "f11-technical-seo-runtime", results);
});
