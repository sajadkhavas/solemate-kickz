...
async function run(baseUrl) {
  for (const path of ["/", "/about", "/brands", "/products", "/product/1"]) {
    ...
    record(
      `${path}: remains reachable with invalid Site URL`,
    ...
    record(
      `${path}: canonical omitted without valid Site URL`,
      canonicalValues(page.head).length === 0,
      canonicalValues(page.head),
    );
    record(
      `${path}: og:url omitted without valid Site URL`,
      metaValues(page.head, "property", "og:url").length === 0,
      metaValues(page.head, "property", "og:url"),
    );
    record(
      `${path}: rejected localhost never leaks into head`,
      !/localhost:9999/i.test(page.head),
      null,
    );
  }
  ...
  record(
    "invalid Site URL robots still allows crawling",
  ...
  record(
    "invalid Site URL sitemap stays valid XML",
    /<urlset xmlns="http:\/\/www\.sitemaps\.org\/schemas\/sitemap\/0\.9">[\s\S]*<\/urlset>/.test(
      sitemapBody,
    ),
    sitemapBody,
  );
  ...
  record(
    "invalid Site URL infrastructure never echoes localhost",
    !/localhost:9999/i.test(`${robotsBody}\n${sitemapBody}`),
    null,
  );
