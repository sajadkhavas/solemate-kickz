...
const incidentRunbooks = read("docs/production/P12-INCIDENT-RUNBOOKS.md");
...
const ownership = read("scripts/deployment/bootstrap-p12-ownership.sh");
...
for (const marker of [
  ...
  if (!incidentRunbooks.includes(marker)) failures.push(`incident runbooks missing ${marker}`);
}
for (const marker of [
...
if (!limits.includes("limit_req_zone $binary_remote_addr")) {
  failures.push("nginx request-limit zone missing");
}
for (const marker of [
...
for (const marker of [
  ...
  "useradd --system",
  "0640",
  "GLOBAL_GIT_SAFE_DIRECTORY_WILDCARD_FORBIDDEN",
]) {
  if (!ownership.includes(marker)) failures.push(`ownership bootstrap missing ${marker}`);
}
for (const marker of [
  ...
  "nginx -t",
  "sole:production:check --json --connections",
  "FRONTEND_LOOPBACK_4173",
  "v22.23.1",
  "1.3.14",
  "MEMORY_P00_MINIMUM",
  "GIT_SAFE_DIRECTORY_WILDCARD_ABSENT",
  "sha256sum",
]) {
...
for (const forbidden of ["printenv", "/proc/1/environ", "cat $ENV", 'cat "$ENV', "ps eww"]) {
  if (evidence.includes(forbidden)) {
    failures.push(`server evidence runner contains unsafe environment disclosure: ${forbidden}`);
  }
}
for (const marker of [
  "SOLE_P12_REHEARSAL",
  "INACTIVE_ONLY",
  "mysql-restore-drill.sh",
  "mktemp -d /var/tmp/sole-p12-symlink",
...
for (const forbidden of [
  ...
  if (prepare.includes(forbidden)) {
    failures.push(`inactive frontend preparation must not activate services: ${forbidden}`);
  }
}
...
if (!verifier.includes("p12-production-readiness")) {
  failures.push("cumulative verifier must require P12 evidence");
}
...
fs.writeFileSync(
  path.join(root, "artifacts/audits/p12-production-readiness-audit.json"),
