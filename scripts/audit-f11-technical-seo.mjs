...
record("JSON-LD serializer escapes script-breaking less-than characters", config.includes("safeJsonLd") && /replace\(\/<\/g/.test(config) && config.includes("u003c"), null);
record("indexation matrix explicitly covers every route", routeSet.every((route) => head.includes(`"${route}"`)) && head.includes('"/products": "demo-catalog"') && head.includes('"/product/$id": "demo-product"'), null);
record(
...
record("demo catalog and products are held out of index and sitemap", head.includes('pathname !== "/products"') && head.includes('{ name: "robots", content: "noindex, follow" }') && serverSeo.includes('SITEMAP_PATHS = ["/", "/about", "/brands"]'), null);
record("canonical and OG URL are emitted only from validated Site URL", head.includes("const canonical = toSiteUrl") && head.includes('links.push({ rel: "canonical", href: canonical })') && head.includes('{ property: "og:url", content: canonical }'), null);
record("route-specific title description OG and Twitter metadata exist", head.includes('name: "description"') && head.includes('property: "og:title"') && head.includes('property: "og:description"') && head.includes('name: "twitter:card"') && head.includes('name: "twitter:title"') && head.includes('name: "twitter:description"'), null);
record("structured data is minimal and SSR-head managed", head.includes('type: "application/ld+json"') && head.includes('"@type": "WebSite"') && head.includes('"@type": "BreadcrumbList"') && head.includes("safeJsonLd"), null);
...
record("no fake merchant or review structured data", forbiddenSchema.every((key) => !head.includes(`\"${key}\"`)), forbiddenSchema.filter((key) => head.includes(`\"${key}\"`)));
record("robots and sitemap are server-served without route-tree additions", serverEntry.includes("createSeoInfrastructureResponse") && serverSeo.includes('url.pathname === "/robots.txt"') && serverSeo.includes('url.pathname === "/sitemap.xml"') && serverSeo.includes('"Allow: /"') && !serverSeo.includes('"Disallow: /"'), null);
record("4xx and 5xx responses receive X-Robots-Tag noindex", serverEntry.includes("withErrorNoindex") && serverSeo.includes('headers.set("x-robots-tag", "noindex, follow")'), null);
record("SEO heads are installed centrally without generated route edits", router.includes("installSeoRouteHeads();") && router.includes("routeTree"), null);
record("invalid products use TanStack notFound semantics", productRoute.includes("throw notFound()") && productRoute.includes("SHOES.find"), null);
record("catalog product discovery remains crawlable through real Links", shoeCard.includes("<Link") && shoeCard.includes('to: "/product/$id"'), null);
record("Persian RTL document contract remains intact", rootRoute.includes('<html lang="fa" dir="rtl"'), null);
record(
  ...
  catalogBehavior.includes("async function activateVisibleText") &&
    ...
    catalogBehavior.includes("await activateVisibleText(client, '[data-testid=\"mobile-filter-dialog\"] button', \"Nike\")") &&
...
record("handoff records required F11 policies and validation sections", /Baseline SHA/.test(handoff) && /Route indexation matrix/.test(handoff) && /Canonical policy/.test(handoff) && /Site URL configuration/.test(handoff) && /Query\/facet policy/.test(handoff) && /robots\.txt policy/.test(handoff) && /Sitemap policy/.test(handoff) && /Structured data policy/.test(handoff) && /SSR validation/.test(handoff) && /Regression audit/.test(handoff), handoff ? "content checked" : "missing");
