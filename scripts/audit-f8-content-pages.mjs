...
function labels(source) {
  const ids = [...source.matchAll(/<input[\s\S]*?\bid="([^"]+)"[\s\S]*?>/g)].map(
    (match) => match[1],
  );
...
record("Foundation SHA", git("merge-base", "--is-ancestor", FOUNDATION, "HEAD").status === 0, {
...
record(
  "No placeholder URL",
  !/href=["'](?:#|javascript:|https?:\/\/(?:example|test)\.)/i.test(combined),
  null,
);
record(
  "No fabricated policy or metric",
  !/(مشتری راضی|سال سابقه|تضمین اصالت|ارسال رایگان|بازگشت وجه تضمینی|فروش موفق)/i.test(combined),
  null,
);
...
record(
  "Auth truthfulness",
  /Backend/.test(auth) && /متصل نیست|بدون Backend/.test(auth) && /هیچ حساب، نشست/.test(auth),
  null,
);
record(
  "No fake auth success or OTP",
  !/(toast\.success|signIn\(|useStore\(|navigate\s*\(|InputOTP|generateOtp|setOtp)/i.test(auth),
  null,
);
record(
  "No password storage",
  !/(localStorage|sessionStorage)\s*\.(?:setItem|removeItem)|persist\s*\(/i.test(auth),
  null,
);
...
record(
  "Input autocomplete",
  /autoComplete="email"/.test(auth) &&
    /current-password/.test(auth) &&
    /new-password/.test(auth) &&
    /autoComplete="name"/.test(auth),
  null,
);
record(
  "Error semantics",
  /aria-invalid=/.test(auth) && /aria-describedby=/.test(auth) && /role="alert"/.test(auth),
  null,
);
record(
  "Double-submit prevention",
  /busyRef\.current/.test(auth) && /loading=\{submitting\}/.test(auth),
  null,
);
record(
  "Backend unavailable state",
  /auth-backend-status/.test(auth) && /ورود واقعی انجام نمی‌شود/.test(auth),
  null,
);
...
record(
  ...
  /\.\.\.new Set\(BRANDS\)/.test(brands) &&
    /SHOES\.filter\(\(shoe\) => shoe\.brand === name\)/.test(brands) &&
    /data-product-count=\{brand\.count\}/.test(brands),
  null,
);
record(
...
const contactLinks = [...combined.matchAll(/href="((?:mailto:|tel:)[^"]+)"/g)].map(
  (match) => match[1],
);
record(
  "Contact link validity",
  contactLinks.every((href) =>
    href.startsWith("mailto:")
      ? /^mailto:[^\s@]+@[^\s@]+\.[^\s@]+$/i.test(href)
      : /^tel:\+?[0-9][0-9 -]{5,}$/i.test(href),
  ),
  contactLinks,
);
...
record(
  "Table semantics",
  tables.every((route) => /<caption\b/.test(sources[route]) && /<th\b/.test(sources[route])),
  tables,
);
record(
  "Static touch targets",
  /min-h-11/.test(brands) && /min-h-1[12]/.test(auth) && /size="lg"/.test(sources.about),
  null,
);

const trackedArtifacts = git("ls-files", "artifacts/runtime", "artifacts/visual-qa")
  .stdout.trim()
  .split("\n")
  .filter(Boolean);
...
record(
  "Handoff presence",
  fs.existsSync(path.join(ROOT, "docs/handoffs/F8-CONTENT-PAGES.md")),
