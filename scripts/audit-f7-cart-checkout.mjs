...
const branch =
  ...
  "detached";
record("branch is controlled", branch === OWNER_BRANCH || branch === INTEGRATION_BRANCH, {
  expected: [OWNER_BRANCH, INTEGRATION_BRANCH],
  actual: branch,
});
record(
...
record(
  "delivery and payment remain explicit backend boundaries",
...
record(
  "Order Review exists and real-order action is disabled",
  checkoutRoute.includes("checkout-review-item") &&
    ...
    /<button[\s\S]*?(?:data-testid="checkout-final-action"[\s\S]*?disabled|disabled[\s\S]*?data-testid="checkout-final-action")/.test(
      checkoutRoute,
    ),
);
record(
  "Checkout is registered in the generated route tree",
...
record(
  "F2 generated-route gate remains strict except the authorized Checkout addition",
...
record(
  "F7 commands are permanent and included in check",
...
record(
  "Frontend CI runs every F7 permanent gate",
...
record("cumulative verifier requires F7 evidence", cumulative.includes('"f7-cart-checkout"'));
record(
